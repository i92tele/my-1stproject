...
class DatabaseManager:
    ...
    async def create_ad_slot(self, user_id: int, slot_number: int, category: str = 'general') -> Optional[int]:
        """Create a new ad slot for a user with worker assignment."""
        async with self._lock:
            try:
                conn = sqlite3.connect(self.db_path, timeout=15)
                conn.execute("PRAGMA busy_timeout=15000;")
                ...
                # Assign worker based on slot number (round-robin for now)
                # In the future, this could be based on worker availability or category
                assigned_worker_id = ((slot_number - 1) % 2) + 1  # Worker 1 or 2 (we have 2 workers)
                ...
                self.logger.info(f"Created ad slot {slot_id} for user {user_id} with category '{category}' assigned to worker {assigned_worker_id}")
                return slot_id
            except Exception as e:
    ...
    async def record_payment(self, user_id: int, payment_id: str, amount: float, 
                           ...
                           required_confirmations: Optional[int] = None) -> bool:
            ...
            try:
                ...
                cursor.execute('''
                    INSERT INTO payments (
                        ...
                        created_at, expires_at, timeout_minutes
                    )
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    payment_id, user_id, amount, currency, crypto_type, status,
                    payment_provider, provider_payment_id,
                    expected_amount_crypto, pay_to_address, network, required_confirmations,
                    datetime.now(), expires_at, timeout_minutes
                ))
                conn.commit()
                conn.close()
                return True
