#!/usr/bin/env python3
"""
Immediate Admin UI Fix (Corrected)
    ...
    async def get_admin_slot_destinations(self, slot_id: int) -> List[Dict[str, Any]]:
        ...
        async with self._get_lock():
            try:
                ...
                cursor.execute('''SELECT mg.* FROM managed_groups mg
                    INNER JOIN admin_slot_destinations asd ON mg.id = asd.destination_id
                    WHERE asd.slot_id = ?
                    ORDER BY mg.category, mg.group_name''', (slot_id,))
    ...
    async def update_admin_slot_content(self, slot_number: int, content: str) -> bool:
        ...
        async with self._get_lock():
            try:
                ...
                cursor.execute('''UPDATE admin_ad_slots 
                    SET content = ?, updated_at = datetime('now')
                    WHERE id = ?''', (content, slot_number))
        ...
        async with self._get_lock():
            try:
                ...
                cursor.execute('''UPDATE admin_ad_slots 
                    ...
                    WHERE id = ?''', (1 if is_active else 0, slot_number))
...
def main():
    """Main function."""
    logger.info("=" * 60)
    logger.info("🔧 IMMEDIATE ADMIN UI FIX (CORRECTED)")
    logger.info("=" * 60)
