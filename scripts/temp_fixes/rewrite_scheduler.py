#!/usr/bin/env python3
"""Rewrite scheduler with proper indentation"""
...
class AutomatedScheduler:
    ...
    async def _get_active_ad_slots(self) -> List[Dict]:
        ...
        try:
            # This would query the database for active ad slots
    ...
    async def _get_destinations(self) -> List[Dict]:
        ...
        try:
            # This would query the database for destinations
            # For now, return empty list
            return []
        except Exception as e:
...
# Write the fixed scheduler
with open('scheduler/core/scheduler.py', 'w') as f:
    f.write(scheduler_content)

print("✅ Scheduler rewritten with proper indentation!")
