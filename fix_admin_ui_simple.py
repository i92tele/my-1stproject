...
This script adds the missing admin functions to the database manager.
"""
...
def add_missing_admin_functions():
    ...
    try:
        ...
        if "from datetime import datetime" not in content:
            # Find the imports section and add datetime import
            import_line = "from datetime import datetime\n"
            content = content.replace("from typing import", "from typing import\nfrom datetime import datetime")
            logger.info("✅ Added datetime import")
        
        # Add the missing functions at the end of the class
        functions_to_add = '''
    ...
    async def get_admin_ad_slot(self, slot_number: int) -> Optional[Dict[str, Any]]:
        ...
        async with self._get_lock():
            try:
                ...
                cursor.execute("SELECT id, slot_number, content, destinations, is_active, created_at, updated_at FROM admin_ad_slots WHERE slot_number = ?", (slot_number,))
    ...
    async def get_admin_slot_destinations(self, slot_id: int) -> List[Dict[str, Any]]:
        ...
        async with self._get_lock():
            try:
                ...
                cursor.execute("SELECT destination_id, destination_name, slot_id, slot_type FROM slot_destinations WHERE slot_id = ? AND slot_type = 'admin'", (slot_id,))
    ...
    async def update_admin_slot_content(self, slot_id: int, content: str) -> bool:
        ...
        async with self._get_lock():
            try:
                ...
                cursor.execute("UPDATE admin_ad_slots SET content = ?, updated_at = ? WHERE id = ?", (content, datetime.now(), slot_id))
    ...
    async def update_admin_slot_status(self, slot_id: int, is_active: bool) -> bool:
        ...
        async with self._get_lock():
            try:
                ...
                cursor.execute("UPDATE admin_ad_slots SET is_active = ?, updated_at = ? WHERE id = ?", (is_active, datetime.now(), slot_id))
...
'''
        ...
        # Find the end of the class (before the last function)
        # Look for the last function in the class
        last_function_pattern = r'(\s+async def [^:]+:.*?\n\s+return [^\n]+\n)'
        matches = list(re.finditer(last_function_pattern, content, re.DOTALL))
...
def main():
    """Main function."""
    logger.info("=" * 60)
    logger.info("🔧 SIMPLE ADMIN UI FIX")
    logger.info("=" * 60)
