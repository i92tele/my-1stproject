...
class HealthChecker:
    ...
    async def check_database_connection(self):
        ...
        try:
            ...
            user_created = await db.create_user(test_user_id, "health_check_user", "Health Check")
            
            # Check if tables exist by trying to access them
            try:
                await db.get_user(test_user_id)
                await db.get_managed_groups()
                await db.get_bot_statistics()
    ...
    async def check_critical_user_flows(self):
        ...
        try:
            ...
            user_created = await db.create_user(test_user_id, "flow_test_user", "Flow Test")
            
            # Flow 2: Subscription activation
            subscription_activated = await db.activate_subscription(test_user_id, 'basic', 7)
            
            # Flow 3: Ad slot creation
            ad_slots = await db.get_or_create_ad_slots(test_user_id, 'basic')
            
            # Flow 4: Payment creation
            payment_data = {
                'payment_id': f"health_check_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            ...
            if all([user_created, subscription_activated, ad_slots, payment_created]):
                status = 'PASS'
                issues = []
                fixes = []
            else:
