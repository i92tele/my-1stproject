    ...
    async def get_admin_slot_destinations(self, slot_id: int) -> List[Dict[str, Any]]:
        ...
        async with self._get_lock():
            try:
                ...
                cursor.execute('''
                    SELECT mg.* FROM managed_groups mg
                    INNER JOIN admin_slot_destinations asd ON mg.id = asd.destination_id
                    WHERE asd.slot_id = ?
                    ORDER BY mg.category, mg.group_name
                ''', (slot_id,))
        ...
        async with self._get_lock():
            try:
                ...
                cursor.execute('''
                    ...
                    WHERE id = ?
                ''', (1 if is_active else 0, slot_number))
...
def main():
    """Main function."""
    logger.info("=" * 60)
    logger.info("🔧 IMMEDIATE ADMIN UI FIX")
    logger.info("=" * 60)
