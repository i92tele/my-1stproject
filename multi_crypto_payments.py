...
class MultiCryptoPaymentProcessor:
    ...
    async def create_payment_request(self, user_id: int, tier: str, crypto_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Record payment in database (extended fields if available)
    ...
    async def _create_direct_payment(self, payment_id: str, amount_usd: float, crypto_type: str) -> Dict[str, Any]:
        """Create a direct payment for all supported cryptocurrencies."""
        try:
            if crypto_type == 'TON':
                ...
                ton_address = getattr(self.config, 'ton_address', '') or os.getenv('TON_ADDRESS', '')
                if not ton_address:
                ...
                btc_address = getattr(self.config, 'btc_address', '') or os.getenv('BTC_ADDRESS', '')
                if not btc_address:
                ...
                eth_address = getattr(self.config, 'eth_address', '') or os.getenv('ETH_ADDRESS', '')
                if not eth_address:
                ...
                payment_url = f"ethereum:{eth_address}?value={int(amount_crypto * 1e18)}&data={payment_id}"
                return {
                    'amount_crypto': amount_crypto,
                    'payment_url': payment_url,
                    'provider_payment_id': None,
                ...
                raise Exception(f"Direct payment not supported for {crypto_type}")
    ...
    async def _verify_direct_payment(self, payment: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if crypto_type == 'TON':
                return await self._verify_ton_payment(payment, required_amount, required_conf, memo)
            elif crypto_type == 'BTC':
                return await self._verify_btc_payment(payment, required_amount, required_conf, memo)
            elif crypto_type == 'ETH':
                return await self._verify_eth_payment(payment, required_amount, required_conf, memo)
            elif crypto_type in ['USDT', 'USDC']:
                return await self._verify_erc20_payment(payment, required_amount, required_conf, memo)
            else:
    ...
    async def _verify_ton_payment(self, payment: Dict[str, Any], required_amount: float, required_conf: int, memo: str) -> bool:
        ...
        try:
            ...
            for i, tx in enumerate(txs):
                ...
                # Only log if it's a potential match to reduce clutter
                if is_inbound and comment and amount_ton > 0:
                    self.logger.info(f"TX {i+1}: amount={amount_ton}, comment='{comment}', confirmations={confirmations}")
                ...
                if amount_ton + 1e-9 < min_required:
                    continue
    ...
    async def _verify_btc_payment(self, payment: Dict[str, Any], required_amount: float, required_conf: int, memo: str) -> bool:
        """Verify Bitcoin payment using mempool.space API."""
        try:
            btc_address = payment.get('pay_to_address') or getattr(self.config, 'btc_address', '') or os.getenv('BTC_ADDRESS', '')
            if not btc_address:
                self.logger.error("BTC address not configured for verification")
                return False
            
            # Fetch recent transactions from mempool.space
            txs = await self._fetch_btc_recent_txs(btc_address, limit=10)
            self.logger.info(f"Found {len(txs)} recent BTC transactions for {btc_address}")
            
            match = None
            for i, tx in enumerate(txs):
                # Extract transaction data
                amount_btc = self._extract_btc_amount(tx)
                is_inbound = self._is_btc_inbound(tx, btc_address)
                confirmations = self._extract_btc_confirmations(tx)
                
                # Only log potential matches
                if is_inbound and amount_btc > 0:
                    self.logger.info(f"BTC TX {i+1}: amount={amount_btc}, confirmations={confirmations}")
                
                if not is_inbound:
                    continue
                # For BTC, we match by amount only (no memo support in basic implementation)
                min_required = required_amount * (1.0 - max(0.0, min(self.payment_tolerance, 0.2)))
                if amount_btc + 1e-9 < min_required:
                    continue
                if confirmations < required_conf:
                    continue
                self.logger.info(f"✅ BTC TX {i+1}: MATCH FOUND! amount={amount_btc}")
                match = tx
                break
    ...
    async def _verify_eth_payment(self, payment: Dict[str, Any], required_amount: float, required_conf: int, memo: str) -> bool:
        ...
        try:
            eth_address = payment.get('pay_to_address') or getattr(self.config, 'eth_address', '') or os.getenv('ETH_ADDRESS', '')
            if not eth_address:
                self.logger.error("ETH address not configured for verification")
                return False
            
            # Fetch recent transactions from Etherscan
            txs = await self._fetch_eth_recent_txs(eth_address, limit=10)
            ...
            for i, tx in enumerate(txs):
                ...
                # Only log potential matches
                if is_inbound and amount_eth > 0:
                    self.logger.info(f"ETH TX {i+1}: amount={amount_eth}, confirmations={confirmations}")
                
                if not is_inbound:
                    continue
                # For ETH, we match by amount only (no memo support in basic implementation)
                min_required = required_amount * (1.0 - max(0.0, min(self.payment_tolerance, 0.2)))
                if amount_eth + 1e-9 < min_required:
                    continue
                if confirmations < required_conf:
                    continue
                self.logger.info(f"✅ ETH TX {i+1}: MATCH FOUND! amount={amount_eth}")
                match = tx
                break
    ...
    async def _verify_erc20_payment(self, payment: Dict[str, Any], required_amount: float, required_conf: int, memo: str) -> bool:
        """Verify ERC-20 token payment (USDT/USDC) using Etherscan API."""
        try:
            eth_address = payment.get('pay_to_address') or getattr(self.config, 'eth_address', '') or os.getenv('ETH_ADDRESS', '')
            if not eth_address:
            ...
            for i, tx in enumerate(txs):
                ...
                if not is_inbound:
                    continue
                # For ERC-20, we match by amount only
                min_required = required_amount * (1.0 - max(0.0, min(self.payment_tolerance, 0.2)))
                if amount_token + 1e-9 < min_required:
                    continue
                if confirmations < required_conf:
                    continue
                self.logger.info(f"✅ {crypto_type} TX {i+1}: MATCH FOUND! amount={amount_token}")
                match = tx
                break
    ...
    def _extract_btc_amount(self, tx: Dict[str, Any]) -> float:
    ...
    async def _activate_subscription(self, user_id: int, payment_id: str) -> bool:
        ...
        try:
            ...
            if success:
                self.logger.info(f"✅ Activated subscription for user {user_id} with tier {tier}")
    ...
    async def _get_crypto_price(self, crypto_type: str) -> Optional[float]:
        ...
        try:
            ...
            if (crypto_type in self.price_cache and 
                ...
                datetime.now().timestamp() - self.price_cache_time[crypto_type] < self.price_cache_duration):
                ...
                async with session.get(url) as response:
                    if response.status == 200:
                        ...
                        if coin_id in data and 'usd' in data[coin_id]:
                            ...
                            self.price_cache_time[crypto_type] = datetime.now().timestamp()
                            return price
