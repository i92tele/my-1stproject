...
class CallbackHandler:
    ...
    async def handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        elif data.startswith("pay:") or data.startswith("check_payment:"):
            await self._handle_payment(update, context, data)
    ...
    async def _handle_payment(self, update: Update, context: ContextTypes.DEFAULT_TYPE, data: str):
        """Handle payment-related callbacks."""
        if data.startswith("pay:"):
            tier = data.replace("pay:", "")
            await self._process_payment_request(update, context, tier)
    ...
    async def _process_payment_request(self, update: Update, context: ContextTypes.DEFAULT_TYPE, tier: str):
        """Process payment request for selected tier."""
        ...
        try:
            ...
            if payment_request.get('success') is False:
                await self._send_fallback_message(update, context, f"Error creating payment: {payment_request.get('error')}")
                return
            ...
            await self._send_fallback_message(update, context, f"Error creating payment: {str(e)}")
